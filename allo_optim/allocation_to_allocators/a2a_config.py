...
class A2AConfig(BaseModel):
    ...
    error_estimator_names: List[str] = Field(
        default=['expected_outcome', 'sharpe_ratio'],
        description="List of error estimator names to use"
    )

    # Orchestrator-specific params
    n_simulations: int = Field(
        default=100,
        description="Number of simulations for Monte Carlo/Bootstrap"
    )
    n_particles: int = Field(
        default=30,
        description="Number of particles for PSO optimization"
    )
    n_pso_iterations: int = Field(
        default=50,
        description="Number of PSO iterations"
    )
    meta_model_type: str = Field(
        default='lightgbm',
        description="Meta-model type for stacking: 'lightgbm', 'xgboost', etc."
    )
