...
def main():
    """Demo of asset name utilities"""
    # Create sample data
    asset_names = ["AAPL", "GOOGL", "MSFT", "TSLA"]
    mu = pd.Series([0.1, 0.12, 0.08, 0.15], index=asset_names)
    cov_data = np.array(
        [[0.04, 0.01, 0.02, 0.01], [0.01, 0.05, 0.015, 0.02], [0.02, 0.015, 0.03, 0.01], [0.01, 0.02, 0.01, 0.08]]
    )
    cov = pd.DataFrame(cov_data, index=asset_names, columns=asset_names)

    # Test validation
    print("Testing asset name validation...")
    validate_asset_names(mu, cov)
    print("✓ Validation passed")

    # Test asset name extraction
    print(f"Asset names from mu: {get_asset_names(mu=mu)}")
    print(f"Asset names from cov: {get_asset_names(cov=cov)}")

    # Test conversion
    print("\nTesting numpy conversion...")
    mu_array, cov_array, names = convert_pandas_to_numpy(mu, cov)
    print(f"Converted to numpy: mu shape={mu_array.shape}, cov shape={cov_array.shape}")

    # Test weight creation
    weights = np.array([0.25, 0.25, 0.25, 0.25])
    weights_series = create_weights_series(weights, asset_names)
    print(f"Created weights series:\n{weights_series}")

    # Test validation
    validate_weights_series(weights_series)
