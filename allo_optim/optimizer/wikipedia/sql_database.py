...
def _legacy_initialize_database():
	...
	with sqlite3.connect(DATABASE_PATH) as conn:
		...
		conn.execute(
			"""
            CREATE TABLE IF NOT EXISTS stock_volumes (
                date DATE PRIMARY KEY
            )
        """
		)
...
def load_data(
	...
	for df_name, df in [
		...
		df = df.reindex(complete_date_range, fill_value=np.nan)
...
#     # TODO implement this function, copy from load_data
#     pass
