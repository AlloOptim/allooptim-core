...
if __name__ == "__main__":
	...
	returns = np.random.randn(n_days, n_assets) * 0.01 + 0.0002
	prices = 100 * np.exp(np.cumsum(returns, axis=0))

	logger.debug("=" * 60)
	logger.debug("FAST PORTFOLIO OPTIMIZER - Daily Rebalancing")
	logger.debug("=" * 60)

	# Initialize and train
	optimizer = FastPortfolioOptimizer(n_assets=n_assets, lookback=252, decay=0.94, risk_aversion=2.0)

	# Initial training (do this once)
	train_prices = prices[:400]
	train_returns = returns[:400]

	import time

	start = time.time()
	optimizer.train(train_prices, train_returns)
	train_time = time.time() - start
	logger.debug(f"\nInitial training time: {train_time:.2f} seconds")

	# Daily update and optimization (this is what runs daily)
	logger.debug("\n" + "=" * 60)
	logger.debug("DAILY EXECUTION SIMULATION")
	logger.debug("=" * 60)

	for day in range(400, min(405, n_days)):
		start = time.time()

		# Get recent data
		recent_prices = prices[day - 252 : day + 1]
		new_return = returns[day : day + 1]

		# Incremental update (optional, run weekly)
		if day % 5 == 0:
			optimizer.incremental_update(recent_prices, new_return)

		# Predict and optimize
		weights = optimizer.predict(recent_prices[:-1])

		exec_time = time.time() - start

		logger.debug(f"\nDay {day}:")
		logger.debug(f"  Execution time: {exec_time*1000:.1f}ms")
		logger.debug(f"  Top 3 positions: {np.argsort(weights)[-3:][::-1]}")
		logger.debug(f"  Concentration: {np.max(weights)*100:.1f}%")
