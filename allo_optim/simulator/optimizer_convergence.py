...
def _validate_allocation_vector(allocation: np.ndarray, name: str) -> None:
    """Validate allocation vector is properly normalized and contains no NaN"""
    validate_no_nan(allocation, f"{name} allocation")

    allocation_sum = np.sum(allocation)
    # if not np.isclose(allocation_sum, 1.0, rtol=1e-6):
    #     raise ValueError(f"{name} allocation sum {allocation_sum} != 1.0: {allocation}")

    # if np.any(allocation < -1e-6):
    #     raise ValueError(f"{name} allocation contains negative values: {allocation}")
...
def _simulate_optimization(
    ...
    try:
        ...
        validate_no_nan(mu_hat, f"simulated mu")
        validate_no_nan(cov_hat, f"simulated cov")
        ...
        for i, optimizer in enumerate(optimizer_list):
            ...
            _validate_allocation_vector(allocation[i, :], optimizer.name)
        ...
        validate_no_nan(expected_returns, f"error estimate for expected returns.")
