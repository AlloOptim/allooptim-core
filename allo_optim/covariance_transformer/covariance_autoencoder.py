...
if __name__ == "__main__":
	logger.debug("Improved Autoencoder with Synthetic Training Data - Educational Demo")
	logger.debug("=" * 80)
	logger.debug("🎲 Now supports synthetic covariance matrix generation for training!")
	logger.debug("⚠️ WARNING: Even with synthetic data, this remains educationally focused")
	logger.debug("⚠️ due to fundamental sample complexity limitations.")
	logger.debug()

	# Demo with small example using synthetic data
	...
	autoencoder = AutoencoderCovarianceTransformer(
		n_assets=n_assets,
		hidden_dims=[16, 8, 4],  # Small architecture
	...
	try:
		...
		autoencoder.fit(df_prices=None, use_synthetic=True, n_synthetic_samples=n_samples)

		logger.debug("\n✅ Training completed successfully!")

		# Test reconstruction
		logger.debug("\n🔍 Testing reconstruction on random matrix...")
		np.random.seed(42)
		A = np.random.randn(n_assets, n_assets)
		test_cov = A @ A.T + 0.1 * np.eye(n_assets)

		denoised = autoencoder.transform(test_cov, n_observations=100)
		metrics = autoencoder.get_reconstruction_metrics()

		logger.debug(f"   Reconstruction correlation: {metrics.get('correlation', 0):.4f}")
		logger.debug(f"   Relative error: {metrics.get('relative_error', 0):.4f}")

	except Exception as e:
		logger.debug(f"⚠️ Demo error (expected with limited resources): {e}")
